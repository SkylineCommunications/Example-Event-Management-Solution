...
namespace ExampleEventManagerEventIntelligence
{
	...
	using ExampleEventManager_EventIntelligence;
	...
	public class Script
	{
		...
		private void RunSafe(IEngine engine)
		{
			...
			var allEvents = Tools.RetrieveBigOrFilter(
				context.ModelIdentifiers,
				id => EventExposers.Identifier.Equal(id),
				filter => eventHelper.Events.Read(filter).ToList());


			engine.Log("ModelIdentifiers: " + String.Join(",", context.ModelIdentifiers));
			engine.Log("Prompt: " + context.Prompt);
			...
			var resultInstructions = SecureNewtonsoftDeserialization.DeserializeObject<ResultInstructions>(analysisResultInstructions);
			...
			switch (resultInstructions.HTTPVERB)
			{
				...
				case "POST":
					...
					{
						eventHelper.Events.CreateOrUpdate(SecureNewtonsoftDeserialization.DeserializeObject<List<Event>>(resultInstructions.HTTPBODY));
					}
					else
					{
						eventHelper.Events.Create(SecureNewtonsoftDeserialization.DeserializeObject<Event>(resultInstructions.HTTPBODY));
					}
					break;
				case "PUT":
					if (resultInstructions.HTTPBODY.StartsWith("["))
					...
					{
						eventHelper.Events.Update(SecureNewtonsoftDeserialization.DeserializeObject<Event>(resultInstructions.HTTPBODY));
					}
					break;
				case "DELETE":
